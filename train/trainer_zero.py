...
class Trainer_zero:
    ...
    def _init_dataset(self):
        ...
        for val_set_cfg in self.cfg['val_set_list']:
            ...
            # merge main cfg so pose_scale etc. are available to val database
    ...
    def run(self):
        ...
        if os.path.exists(self.pth_fn):
            ...
            if latest_step >= 200000:
                print(f'[NU-NeRF zero_thickness] {self.model_name}: checkpoint at step {latest_step} (>= 200000); skipping training.')
                return
    ...
    def _init_logger(self):
        self.logger = Logger(
            self.model_dir,
            use_wandb=self.cfg.get('use_wandb', True),
